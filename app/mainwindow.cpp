...
void MainWindow::init(){
    ...
    this->client->setRecieveFunc([this](std::string msg){
        this->processServerResponse(msg);
    });
}
...
void MainWindow::connectTo(std::string host, int port){
    if (this->client->isConnected()){
...
std::pair<bool,std::string>  MainWindow::sendMessage(std::string msg){
    if (this->client->isConnected()){
...
void MainWindow::processServerResponse(std::string msg){
    if (msg.find("currentClients")!=std::string::npos){
        int currentClients =0;
        int clientfd;
        sscanf(msg.c_str(),"status=connected; clientfd=%d; currentClients=%d;",&clientfd,&currentClients);
...
void MainWindow::onConnectPress(){
    ...
    if (list.size()!=2){
        QMessageBox::warning(nullptr,"Warning!", "incorrect format, requires host:port");
        return;
    }
